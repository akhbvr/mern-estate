...
export default function Profile() {
  ...
  const handleListingDelete = async (listignId) => {
    try {
      const res = await fetch(`/api/listing/delete/${listignId}`, {method: 'DELETE'})
      const data = await res.json()
      if(data.success === false){
        console.log(data.message);
        return;
      }
      setUserListings((prev) => prev.filter((listing) => listing._id !== listignId))
    } catch (error) {
  ...
  return (
    <div className="p-3 max-w-lg mx-auto">
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-4">
        ...
        <img onClick={() => fileRef.current.click()} src={formData.avatar || currentUser.avatar} alt="profile" className="rounded-full w-24 h-24 object-cover self-center cursor-pointer mt-2" />
